...
class MasterDataController extends Controller
{
    ...
    private function generateKodeBarang($jenisBarang)
    {
        $lastBarang = NamaBarang::where('jenis_barang', $jenisBarang)
            ->orderBy('kode_barang', 'desc')
            ->first();

        $jenis = JenisBarang::where('jenis', $jenisBarang)->first();
        $prefix = strtoupper(substr($jenis->jenis, 0, 3));
    ...
    public function tambahNamaBarang(Request $request)
    {
        ...
        $kodeBarang = $this->generateKodeBarang($request->jenis_barang);
